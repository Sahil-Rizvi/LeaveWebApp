...
<c:url var="firstUrl" value="/leave/getLeavesForUpdationForManager/1" />
<c:url var="lastUrl" value="/leave/getLeavesForUpdationForManager/${pages.totalPages}" />
<c:url var="prevUrl" value="/leave/getLeavesForUpdationForManager/${currentIndex - 1}" />
<c:url var="nextUrl" value="/leave/getLeavesForUpdationForManager/${currentIndex + 1}" />
...
<td>
<input type="radio" name="${leave.responseType}" value="ResponseType.APPROVED"/>Approve
</td>
<td>
<input type="radio" name="${leave.responseType}" value="ResponseType.REJECTED"/>Reject
</td>
<td>
<textarea name="${leave.rejectionReason}" ></textarea>
</td>
</tr>   
</c:forEach>
</table>
...
//	var btn = document.getElementsByName("hisTeam")[0];
	...
	$('[name="hisTeam"]').on('click', function() {
		...
		$.ajax({
		type: "GET",
		url: "/LeaveWebApp/leave/getAllLeavesOfSpecificEmployeeTeamMates/1", 
		data:{"emp":emp},
		success: function(response){
			$('#new1').html(response);
