...
<c:url var="firstUrl" value="/leave/manager/getLeavesRejectedByManager/1" />
<c:url var="baseUrl" value="/leave/manager/getLeavesRejectedByManager" />
	
<c:if test="${not empty pages.totalPages && pages.totalPages>0}">
<c:url var="lastUrl" value="/leave/manager/getLeavesRejectedByManager/${pages.totalPages}" />
<c:if test="${currentIndex - 1 >=1}">
<c:url var="prevUrl" value="/leave/manager/getLeavesRejectedByManager/${currentIndex - 1}" />
</c:if>
<c:if test="${currentIndex + 1 <= pages.totalPages}">
<c:url var="nextUrl" value="/leave/manager/getLeavesRejectedByManager/${currentIndex + 1}" />
</c:if>
</c:if>
</c:if>
