...
<c:url var="firstUrl" value="/leave/emp/getApprovedLeavesOfEmp/1" />
<c:url var="baseUrl" value="/leave/emp/getApprovedLeavesOfEmp/" />
	
<c:if test="${not empty pages.totalPages && pages.totalPages>0}">
<c:url var="lastUrl" value="/leave/emp/getApprovedLeavesOfEmp/${pages.totalPages}" />
<c:if test="${currentIndex - 1 >=1}">
<c:url var="prevUrl" value="/leave/emp/getApprovedLeavesOfEmp/${currentIndex - 1}" />
</c:if>
<c:if test="${currentIndex + 1 <= pages.totalPages}">
<c:url var="nextUrl" value="/leave/emp/getApprovedLeavesOfEmp/${currentIndex + 1}" />
</c:if>
</c:if>
</c:if>
